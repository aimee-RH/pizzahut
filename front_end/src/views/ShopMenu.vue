...
const item = ['/shop/menu/dish', '/shop/menu/card', '/shop/menu/order', '/shop/menu/info']
...
const shopIcon = new URL('@/assets/images/shop.png', import.meta.url)
const name = ref('')

onBeforeMount(() => {
  shopAxios.get("/nav/" + store.shopID)
      .then((res) => {
        name.value = res.data
      })
      .catch(() => {
...
<template>
  <el-container class="main">
    <el-header style="height: 5rem; border-radius: 0.5rem;">
      ...
      <div class="nav_right">
        <img :src="shopIcon" style="margin-right: 1rem" @click="info">
        <div class="info" @click="info">
          <strong class="name">{{ name }}</strong>
          <strong class="rank">门店</strong>
        </div>
...
.el-header {
  background: linear-gradient(120deg, #22ff00 0%, #a9fda2 100%) no-repeat;
  display: flex;
