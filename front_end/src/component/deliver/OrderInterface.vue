...
function receive(id){
  deliverAxios.post("/receiveOrder",{
    orderID: id,
    deliverID: store.deliverID
  })
      .then((res) => {
        if (res.data.code === 1) {
...
<template>
  ...
  <div v-else class="orderBox">
    ...
    <el-table :data="orders" style="width: 90%" max-height="470">
      ...
      <el-table-column>
        <template #default="scope">
          <el-button type="primary" @click="receive(scope.row.编号)" round>接单</el-button>
        </template>
      </el-table-column>
