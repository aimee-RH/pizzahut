...
function use(cardID, shopID) {
    ...
    switch (+cardID) {
        ...
        case 3: ThirdVisible.value = true; break;
    }
            ...
            .then((res) => {
                ...
                dishes.value.forEach((item) => {
                    switch (item.category) {
                        case 'Pizza': pizza.value.push(item); break;
                        case 'Pasta': Spaghetti.value.push(item); break;
                        case 'Steak': steak.value.push(item); break;
                        case 'Drink': beverage.value.push(item); break;
                    }
                })
            })
            .catch(() => {
                ElMessage.error('获取菜品失败，请稍后再试！')
            });
    }
}
...
<template>
      ...
      <el-table :data="cards" style="width: 50rem" max-height="500">
        <el-table-column label="图片" width="250">
          <template #default="scope">
            <img v-if="+scope.row.id === 1" :src="img1" />
            <img v-else-if="+scope.row.id === 2" :src="img2" />
            <img v-else :src="img3" />
        ...
        <el-table-column>
          <template #default="scope">
            <el-button type="primary" @click="use(scope.row.id, scope.row.shop_id)" round>使用</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  <el-dialog v-model="FirstVisible" title="优惠券使用" center>
    <el-form :inline="true" :model="dishForm" class="demo-form-inline">
      <el-form-item label="Steak" label-width="90px">
        <el-select v-model="dishForm.firstDish" placeholder="请选择" clearable>
          <el-option v-for="(item, index) in steak" :key="index" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>

      <el-form-item label="Drink" label-width="90px">
        <el-select v-model="dishForm.secondDish" placeholder="请选择" clearable>
          <el-option v-for="(item, index) in beverage" :key="index" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>

      <el-form-item label="地址" label-width="250px">
        <el-select placeholder="请选择您预留的地址" v-model="dishForm.address">
          <el-option v-for="(item, index) in addresses" :key="index" :label="item" :value="item" />
        </el-select>
      </el-form-item>

      <el-form-item label="备注" label-width="250px">
        <el-input v-model="dishForm.note" placeholder="请输入内容" type="textarea" :rows="2" maxlength="25" show-word-limit />
      </el-form-item>
    </el-form>
    <template #footer>
  ...
  <el-dialog v-model="SecondVisible" title="优惠券使用" center>
    <el-form :inline="true" :model="dishForm" class="demo-form-inline">
      <el-form-item label="Pasta" label-width="90px">
        <el-select v-model="dishForm.firstDish" placeholder="请选择" clearable>
          <el-option v-for="(item, index) in Spaghetti" :key="index" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
  ...
  <el-dialog v-model="ThirdVisible" title="优惠券使用" center>
    <el-form :inline="true" :model="dishForm" class="demo-form-inline">
      <el-form-item label="Pizza" label-width="250px">
        <el-select v-model="dishForm.firstDish" placeholder="请选择" clearable>
          <el-option v-for="(item, index) in pizza" :key="index" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
