...
onBeforeMount(() => {
    orderList.value.user = store.customerID;
    customerAxios.get("/dish")
        .then((res) => {
            dishes.value = res.data.dishes.map(d => ({
                id: d.id,
                name: d.name,
                category: d.category.toLowerCase(),
                description: d.description,
                price: parseFloat(d.originalPrice)
            }))
            console.log('菜品返回结果:', res.data.dishes);

            dishList.value = dishes.value.map(dish => ({
                id: dish.id,
                num: 0,
                money: dish.price
            }))
    ...
    customerAxios.get("/buyDishes/" + store.customerID)
        .then((res) => {
            ...
            shops.value = res.data.shops.map(s => ({
                id: s.id,
                name: s.name
            }));
...
const rowState = ({ row }) => {
    let style = {}
    if (availableDishes.value.includes(row.id)) {
        style = {
...
function confirmPayment() {
    ...
    for (let i = 0, j = 0; i < dishList.value.length; i++) {
        if (dishList.value[i].num > 0) {
            ...
            list.value[j].name = dishes.value[i].name

            orderList.value.total = calFloat(orderList.value.total, paymentList.value[j++].money, '+')
        }
    }
...
function privilege() {
    if (usePrivilege.value) {
        ...
        orderList.value.total = calFloat(orderList.value.total, rebate.value.money, '-')
        orderList.value.privilege = 'Yes'
    } else {
        paymentList.value[rebate.value.dish].money = calFloat(paymentList.value[rebate.value.dish].money, rebate.value.money, '+')
        list.value[rebate.value.dish].money = calFloat(list.value[rebate.value.dish].money, rebate.value.money, '+')
        orderList.value.total = calFloat(orderList.value.total, rebate.value.money, '+')
        orderList.value.privilege = 'No'
    }
}

function submitForm() {
    if (orderList.value.address !== '' && orderList.value.shop !== '') {
        dialogFormVisible.value = false
        if (orderList.value.note === '') {
            orderList.value.note = 'None'
        }
    ...
    <div class="dishBox">
      <el-form>
        <el-form-item>
          ...
          <el-select placeholder="请选择当前营业中的门店" v-model="orderList.shop">
            <el-option
              v-for="(item, index) in shops"
              :key="index"
              :label="item.name"
              :value="item.id"
              @click="alterShop(item.id)"
            />
          </el-select>
        </el-form-item>
      </el-form>
      ...
      >
        <el-table-column label="图片" width="180">
            ...
            <img v-if="scope.row.category === 'pizza'" :src="pizza" />
            <img v-else-if="scope.row.category === 'drink'" :src="beverage" />
            <img v-else-if="scope.row.category === 'salad'" :src="salad" />
            <img v-else-if="scope.row.category === 'pasta'" :src="Spaghetti" />
...
<!-- 
        ...
        <el-table-column
          ...
          :filters="[
            { text: '披萨', value: 'pizza' },
            { text: '饮料', value: 'beverage' },
            { text: '沙拉', value: 'salad' },
            { text: '意面', value: 'spaghetti' },
            { text: '牛排', value: 'steak' }
            ]"
        ...
        <el-table-column label="数量/份">
          <template #default="scope">
            <el-input-number
              size="mini"
              :step="1"
              :min="0"
              :disabled="disabled(scope.row.id)"
              v-model="getListByIndex(scope.$index).num"
            />
          </template>
        </el-table-column>
      </el-table>
  
      <el-button type="success" @click="confirmPayment" style="margin-top: 2rem;" round>
        购买
      </el-button>
    </div>
    ...
    <el-dialog title="请填写订单详细内容" v-model="dialogFormVisible" center>
      <el-form>
        <el-form-item label="会员特权" label-width="200px" v-if="canPrivilege">
          ...
          >
            <template #reference>
              <el-icon style="margin-left: 0.8rem;" size="20px" color="rgb(69, 137, 255)">
                <InfoFilled />
              </el-icon>
            </template>
          </el-popover>
        ...
        <el-form-item label="地址" label-width="200px">
          <el-select placeholder="请选择您预留的地址" v-model="orderList.address">
            <el-option
      ...
      <h1 class="money">
        总金额：
        <span style="font-family: 'Courier New', Courier, monospace;">{{ orderList.total }}</span>
      </h1>
...
.el-table .cell {
  ...
  word-break: break-word;         /* 单词长也不会溢出 */
  line-height: 1.5;
  font-size: 16px;                /* 可选，增加可读性 */
}
