...
function check() {
  ...
  if (form.value.name.length > 20) {
    ElMessage.warning('姓名长度不能超过20个字')
    return false
  }
  if (form.value.phone === '') {
...
function register() {
  ...
  loading.value = true
  shopAxios.post("/register",
      {
        姓名: form.value.name,
        地址: form.value.address,
        电话: form.value.phone,
        密码: form.value.password,
        上班时间: form.value.start,
        下班时间: form.value.end
      })
      .then((res) => {
        if (res.data.code === 1) {
          store.alterShopID(res.data.msg)
          open(res.data.msg)
          loading.value = false
        } else {
  ...
  <div class="box">
    ...
    <div class="right">
      ...
      <form @submit.prevent>
        ...
        <input type="text" class="acc" placeholder="地址" v-model="form.address"/>
        <input type="text" class="acc" placeholder="电话" v-model="form.phone"/>
        <input type="text" class="acc" placeholder="上班时间(HH:MM:SS)" v-model="form.start"/>
        <input type="text" class="acc" placeholder="下班时间(HH:MM:SS)" v-model="form.end"/>
