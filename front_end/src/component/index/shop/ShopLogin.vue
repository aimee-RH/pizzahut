...
function login() {
  if (id.value === '') {
    ...
    document.querySelector('.acc')[0].focus()
  }
  else if (isNaN(id.value)) {
    ElMessage.warning('账号必须是数字')
    document.querySelectorAll('.acc')[0].focus()
  }
  else if (password.value === '') {
    ElMessage.warning('密码不能为空')
    document.querySelectorAll('.acc')[1].focus()
  }
  else if (isNaN(password.value)) {
    ElMessage.warning('密码必须是数字')
    document.querySelectorAll('.acc')[1].focus()
  }
  else if (password.value.length < 6) {
    ElMessage.warning('密码长度需大于或等于6')
    document.querySelectorAll('.acc')[1].focus()
  } else {
    loading.value = true
...
<template>
  <div class="box">
    ...
    <div class="right">
      ...
      <div class="fn">
        <router-link to="/shop/register" style="color: rgb(19,150,8);">注册账号</router-link>
        <router-link to="/shop/alterPassword" style="color: rgb(19,150,8);">忘记密码</router-link>
      </div>
    </div>
  </div>
</template>
...
::selection {
  color: #fff;
  background-color: rgb(91, 224, 58);
